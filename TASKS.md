...
- [ ] Faire évoluer `wiki/lore` vers un guide prioritairement narratif pour les créations dynamiques.
  Références: `test-GAME-2D/narration-module/docs/Cadrage-lore-narratif-dynamique.md`, `Contrat-selection-influences-lore.md` et `Contrat-creation-scene-guidee-lore.md`. Le contrôleur, `scene_creator`, le runtime repository, le catalogue et IndexedDB sont couverts. L'assembleur compose temps, lieu, topologie, faits, position et cycle de scène dans un commit unique. L'adaptateur de production compose désormais contexte de lore, génération et gates avant de déléguer au monde, sans conserver le lease pendant l'appel IA. Prochaine étape concrète: rattacher une scène jouable réelle à une géographie compilée du wiki et fournir son port monde, puis exécuter la recette OpenAI live vers un lieu absent; la fixture isolée de l'auberge ne doit pas recevoir de géographie inventée en dur.
