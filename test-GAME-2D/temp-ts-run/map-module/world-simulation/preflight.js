...
function checkDuplicateIds(layout, issues) {
    const groups = [
...
function validatePopulationProfile(profile, issues, context) {
    if (!profile)
        return;
...
function validateNativeDistrict(layout, district, issues) {
    ...
    (district.cellKeys ?? []).forEach(cellKey => {
        if (layout.cells.some(cell => `${cell.cell.x},${cell.cell.y}` === cellKey))
            return;
        pushIssue(issues, {
            severity: "warning",
...
function getKnownDistrictIds(layout) {
    return new Set(Object.keys(createSimulationSeedFromMapLayout(layout).districts));
}
function isKnownZoneId(layout, knownDistrictIds, zoneId) {
    return (layout.cities.some(entry => entry.id === zoneId) ||
...
function isKnownAnchorTarget(layout, knownDistrictIds, targetKind, targetId, cell) {
    if (targetKind === "cell") {
        if (!cell)
            return false;
...
function getKnownObjectiveTarget(layout, knownDistrictIds, targetKind, targetId) {
    if (!targetKind || !targetId)
        return true;
    if (targetKind === "city")
        return layout.cities.some(entry => entry.id === targetId);
    if (targetKind === "route")
        return layout.paths.some(entry => entry.id === targetId && entry.kind === "road");
    if (targetKind === "region")
        return (layout.governanceRegions ?? []).some(entry => entry.id === targetId);
    if (targetKind === "faction")
        return (layout.simulation?.factions ?? []).some(entry => entry.id === targetId);
    if (targetKind === "district")
        return knownDistrictIds.has(targetId);
    if (targetKind === "place") {
        return layout.cells.some(cell => (cell.locationWikiIds ?? []).includes(targetId));
    }
    return false;
}
function validateFaction(layout, knownDistrictIds, faction, issues) {
...
function validateObjective(layout, knownDistrictIds, objective, issues) {
    ...
    if ((objective.currentPhaseIndex ?? 0) > Math.max(0, (objective.phases ?? []).length - 1) &&
    ...
    if (objective.requiredAnchorType &&
        ownerFaction &&
    ...
    objective.compatibleActionIds.forEach(actionId => {
        const definition = knownActionDefinitions.get(actionId);
        if (!definition) {
            pushIssue(issues, {
                severity: "warning",
        ...
        if (!definition.compatibleObjectives.includes(objective.category)) {
            pushIssue(issues, {
                severity: "warning",
...
function validateDistrictOverride(layout, knownDistrictIds, districtOverride, issues) {
...
function validateCityDistrictModel(layout, knownDistrictIds, issues) {
...
function getRouteById(layout, routeId) {
    return layout.paths.find(path => path.id === routeId && path.kind === "road");
}
function getRouteEndpointKeys(layout, routeId) {
...
function getRegionAnchorCellKey(layout, regionId) {
    ...
    if (region.principalCityId) {
        ...
        if (city)
            return `${city.cell.x},${city.cell.y}`;
    }
    return `${region.labelCell.x},${region.labelCell.y}`;
}
function getPositionNodeId(layout, kind, id, cell) {
    if (kind === "cell") {
...
function validateItinerary(layout, actor, issues) {
...
function validateMobileActor(layout, actor, issues) {
    ...
    if (actor.simulationLevel === "abstract" &&
...
export function runSimulationPreflight(layout) {
    const issues = [];
    const knownDistrictIds = getKnownDistrictIds(layout);
    checkDuplicateIds(layout, issues);
    layout.cities.forEach(city => validatePopulationProfile(city.populationProfile, issues, {
        scope: "layout",
        entityId: city.id,
    ...
    (layout.simulation?.districtOverrides ?? []).forEach(districtOverride => validateDistrictOverride(layout, knownDistrictIds, districtOverride, issues));
