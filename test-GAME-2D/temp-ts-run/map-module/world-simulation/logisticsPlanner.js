...
function getTopObjectiveForFaction(state, faction) {
...
function deriveMobilityRequirement(objective) {
...
function deriveTransportResources(state, faction) {
...
export function reinitialiserRessourcesTransport(state) {
...
function getModeNetwork(mode) {
    return mode === "bateau" ? "river" : mode === "cheval" ? "road" : "foot";
}
function getModeSpeedMultiplier(mode) {
    if (mode === "cheval")
        return 1.3;
    if (mode === "bateau")
        return 1.2;
...
function computeRouteRisk(state, routeIds) {
...
function estimatePlanCost(requirement, mode, routeCount) {
    const modeBase = mode === "cheval" ? 18 : mode === "bateau" ? 22 : 10;
    return Math.round(modeBase +
        requirement.besoinEffectif * 0.6 +
        requirement.besoinCharge * 0.25 +
        routeCount * 2);
}
function computeTransportDemand(requirement, mode) {
...
function buildModePlan(state, faction, requirement, resources, mode) {
...
function selectBestPlan(candidates) {
...
function getAssignableActor(state, factionId, objectifId) {
...
export function buildFactionLogisticsPlans(state) {
...
export function applyFactionLogisticsPlans(state, plans) {
    plans.forEach(plan => {
        if (!plan.faisable || !plan.cibleExecutionRef || !plan.modeRetenu)
            return;
