...
export const GameBoard: React.FC = () => {
  ...
  function minDistanceToAnyEnemy(): number | null {
    ...
    for (const enemy of enemies) {
      const dist = gridDistance(player, enemy);
  ...
  function validateEnemyTargetForAction(
    ...
    const dist = gridDistance(actor, enemy);
  ...
  function handleBoardClick(event: React.MouseEvent<HTMLDivElement>) {
    ...
    if (interactionMode === "inspect-select") {
      const dist = gridDistance(player, { x: targetX, y: targetY });
      ...
      if (!decisions.length) {
        ...
        setEnemies(prevEnemies => {
          ...
          if (path.length === 0) {
            ...
            const distToPlayer = gridDistance(enemy, playerCopy);
  ...
  async function runSingleEnemyTurnV2(activeEnemyId: string) {
    ...
    if (usedFallback) {
      ...
      if (!canSee) {
        ...
        const distToPlayer = gridDistance(activeEnemy, playerCopy);
        ...
        const pickBestRetreatCell = () => {
          ...
          for (let dx = -moveRange; dx <= moveRange; dx++) {
            for (let dy = -moveRange; dy <= moveRange; dy++) {
              ...
              const d = gridDistance({ x, y }, playerCopy);
              if (d > bestDist) {
                bestDist = d;
                best = { x, y };
              }
