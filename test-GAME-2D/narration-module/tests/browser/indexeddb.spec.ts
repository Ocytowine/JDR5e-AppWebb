...
test("campaign-core and bootstrap contracts pass against real Chromium IndexedDB", async ({ page }) => {
  ...
  const failures = [...result.contracts.failures, ...result.bootstrap.failures, ...result.specific.failures];
