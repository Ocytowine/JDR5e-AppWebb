...
class OpenAIMjNarrationGenerator {
    ...
    async generate(input) {
        ...
        const systemPrompt = 'You are a strict narrative game master assistant. Return ONLY valid JSON with keys: selectedIndex (number|null) and reason (string). Select one candidate index from candidates when possible.';
        const userPayload = {
            query: input.query,
