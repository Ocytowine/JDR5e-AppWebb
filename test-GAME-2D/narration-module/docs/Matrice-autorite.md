...
| Faits objectifs et overrides de partie | `CampaignFactDomain` | Scène, monde, acteurs, mémoire, contexte IA | IA, monde, tactique, repos, joueur via actions | Validation par le propriétaire de la propriété puis enregistrement sourcé | `campaign_fact_asserted`, `campaign_fact_replaced`, `campaign_fact_invalidated` | Projections lore `REPLACE` et `WITHHOLD` persistées ; faits libres et cycle complet absents |
...
| Voyage du PJ et rencontres de trajet | `WorldDomain` pour position, temps et tirage; `SceneDomain` pour la mise en scène | Orchestrateur, monde, mémoire, UI | Joueur, simulation, IA pour candidat contextuel | Topologie, règles de voyage, graine stable et validation de création | `travel_started`, `travel_segment_completed`, `travel_encounter_committed`, `travel_interrupted`, `player_arrived` | Mobilité des acteurs monde présente; processus joueur et moteur de rencontre absents |
...
| Relation individuelle, dette et historique social | `SocialKnowledgeDomain` — volet social | Scène, PNJ concerné, contexte IA filtré, journal joueur selon visibilité | IA, actions et événements validés | Règles sociales | `relationship_changed`, `debt_created`, `debt_settled` | Registre initial de proposition/résolution livré ; axes, dettes et historique long absents |
| Disposition temporaire et résolution sociale | `SocialKnowledgeDomain` pour le résultat social; `NarrativeActorDomain` pour motivations et état de l'acteur | Scène, interprètes d'acteurs, UI selon visibilité | Joueur et IA via intentions ou facteurs contextuels | Faisabilité de l'acteur, ruleset social, jet ou arbitrage borné | `social_attempt_resolved`, `disposition_changed`, actes de parole et événements propriétaires | Décisions `ACCEPTED`, `REFUSED`, `CONDITIONAL`, `UNCERTAIN` persistées ; règles sociales complètes absentes |
...
| Intrigue, mission et fil narratif | `SceneDomain` pour l'activité locale, persisté comme agrégat narratif de campagne | Scène, journal joueur filtré, contexte IA | IA, monde, joueur par ses décisions | Règles de cycle narratif | `thread_created`, `thread_advanced`, `thread_resolved`, `thread_abandoned` | Acceptation initiale d'un engagement persistée ; cycle complet de mission et intrigue absent |
...
| Messages validés du fil et transcript | `InteractionLog` comme projection non autoritaire des opérations, résultats et actes de parole committés; `SceneDomain` reste propriétaire des faits de scène établis | UI, reprise, diagnostic | IA après résultats validés pour la prose; événements validés pour les actes durables | Contrôle des locuteurs, fondations et droits de révélation puis complétion technique de l'opération | aucun événement métier pour la prose seule; les actes de parole utilisent leurs événements propriétaires | Route de résumé tactique existante, sans opérations durables ni projection de campagne |
...
- Les PNJ promus et les décisions initiales mission/relation possèdent un runtime
  persistant borné. Les connaissances, dettes, relations longues, scènes et
  cycles d'intrigue complets restent absents.
