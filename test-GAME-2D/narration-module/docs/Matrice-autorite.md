...
| Inventaire, monnaie et équipement du PJ | `CharacterDomain` de campagne avec `InventoryRules` | Scène, tactique, repos, UI | Joueur, IA, récompense, commerce, tactique | Validation inventaire puis transaction coordonnée | `item_acquired`, `item_transferred`, `item_consumed`, `currency_changed` | Inventaire exporté par l'éditeur; transaction de campagne absente |
