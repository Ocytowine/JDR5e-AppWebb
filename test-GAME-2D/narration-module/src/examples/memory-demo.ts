...
const initial = NarrativeRuntime.createInitialState();
...
console.log(
  '[MEMORY DEMO] shortTop ->',
  state.memory.shortTerm.slice(0, 3).map((entry) => ({
...
console.log(
  '[MEMORY DEMO] longTop ->',
  state.memory.longTerm.slice(0, 3).map((entry) => ({
    transitionId: entry.transitionId,
    importance: Number(entry.importance.toFixed(2)),
    ageHours: entry.ageHours
  }))
