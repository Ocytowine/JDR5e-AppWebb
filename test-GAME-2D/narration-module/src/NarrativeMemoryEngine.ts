...
function impactWeight(scope: ImpactScope): number {
  switch (scope) {
    case 'global':
      return 6;
    case 'regional':
...
function timeToHours(block: TimeBlock): number {
  switch (block.unit) {
    case 'hour':
      return block.value;
    case 'day':
...
function isMajor(outcome: RuntimeTransitionOutcome): boolean {
  const { entityType, toState, impactScope } = outcome.result;
  if (impactScope === 'regional' || impactScope === 'global') return true;
  if (entityType === 'quest' && (toState === 'Terminée' || toState === 'Acceptée')) return true;
  if (entityType === 'trama') return true;
  return false;
}
...
function decayEntry(entry: NarrativeMemoryEntry, elapsedHours: number): NarrativeMemoryEntry {
  if (elapsedHours <= 0) return entry;
...
function makeEntry(outcome: RuntimeTransitionOutcome): NarrativeMemoryEntry {
...
function toLongTerm(entry: NarrativeMemoryEntry): NarrativeMemoryEntry {
  return {
    ...entry,
...
export class NarrativeMemoryEngine {
  public apply(state: NarrativeGameState, outcome: RuntimeTransitionOutcome): NarrativeGameState {
    ...
    if (isMajor(outcome)) {
      const already = longTerm.find(
        (item) =>
          item.transitionId === entry.transitionId &&
